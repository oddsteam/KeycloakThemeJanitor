function validatePassword() {
    ...
    if (passwordValue === '' && usernameValue === '') {
        ...
        ValidateLogin(username, 'กรุณากรอกอีเมลหรือชื่อผู้ใช้', true);
        ValidateLogin(password, 'กรุณากรอกรหัสผ่าน', true);
        return false;
    } else {
        clearError(["username", "password", "input-error"]);
        if (usernameValue === '') {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (invalid_) {
        ...
        invalid_.remove();
        ValidateLogin(password, 'อีเมล/ชื่อผู้ใช้ หรือรหัสผ่านไม่ถูกต้อง')
    }
});
...
if (window.location.href.indexOf("reset-credentials") > -1) {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (emailValidate) {
            ...
            ValidateLogin(email, 'กรุณากรอกอีเมล/ชื่อผู้ใช้')
        }
    });
}
...
// on update password state
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        header.textContent = "ตั้งรหัสผ่าน";
...
function validatePassword_UpdatePassword(event) {
    ...
    if (passwordValue === '' || passwordConfirmValue === '') {
        ...
        clearError(["password-new", "password-confirm"]);
        errorMessage = "รหัสผ่านต้องเป็นภาษาอังกฤษและมีความยาวอย่างน้อย 8 ตัวอักษร ตัวพิมพ์ใหญ่หนึ่งตัว, ตัวพิมพ์เล็กหนึ่งตัว และตัวเลขหนึ่งตัว";
        showErrorMessage(passwordConfirm, errorMessage, true);
        return false;
    }
    else {
